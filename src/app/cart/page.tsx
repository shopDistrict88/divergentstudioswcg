...
export default function CartPage() {
  const { items = [], removeItem, updateQuantity, subtotal, itemCount } =
    useCart();
  const cartLabel = String(itemCount).padStart(2, "0");

  return (
    <div className="min-h-screen bg-[#050505] px-4 pb-24 pt-20 md:px-6">
      <div className="mx-auto max-w-[1100px]">
        <p className="label-code text-dirty-white/40">SELECTED PIECES</p>
        <p className="label-code mt-2 text-dirty-white/25">CART ({cartLabel})</p>

        {items.length === 0 ? (
          <div className="mt-20">
            <p className="label-code text-dirty-white/85">Nothing held</p>
            <Link
              href="/collections/"
              className="btn-solid mt-10 inline-flex focus-ring"
            >
              View Collections
            </Link>
          </div>
          ...
          <div className="mt-12 grid gap-14 lg:grid-cols-[1fr_280px]">
                    ...
                    >
                      ...
                      <div className="flex flex-1 flex-col">
                        <div className="flex justify-between gap-2">
                          <div>
                            <p className="text-[12px] uppercase tracking-[0.14em] text-[#E8E6E1]">
                              {item.product.name}
                            </p>
                            <p className="mt-1 label-code text-dirty-white/40">
                              {objectCode(i)} · Size {item.size}
                            </p>
                          </div>
                          <button
                            type="button"
            ...
            <aside className="h-fit border border-dirty-white/10 bg-[#111111] p-6">
              ...
              <p className="label-code mt-4 text-dirty-white/25">
