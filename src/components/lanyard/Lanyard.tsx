  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current) {
      [j1, j2].forEach((ref) => {
        ...
        const b = body as BodyWithLerp;
        if (!b.lerped) {
          b.lerped = new THREE.Vector3().copy(body.translation());
        }
        const clampedDistance = Math.max(
          0.1,
          Math.min(1, b.lerped.distanceTo(body.translation()))
        );
        b.lerped.lerp(
          body.translation(),
      ...
      curve.points[1].copy((j2.current as BodyWithLerp).lerped!);
      curve.points[2].copy((j1.current as BodyWithLerp).lerped!);
      curve.points[3].copy(fixed.current.translation());

      const bandGeo = band.current?.geometry as MeshLineGeometry | undefined;
      bandGeo?.setPoints(curve.getPoints(isMobile ? 16 : 32));
